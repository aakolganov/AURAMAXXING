...
class Lammps_interface:
    ...
    def opt_struc(self, type_opt = "minimize", steps = None, removal = None, max_num_rings=None, start_T=None, final_T=None, max_remove_over=None, FF=None):
...
set             type 1 charge 2.3  # Si charge
set             type 2 charge -1.15 # O charge
...
write_data      final_struc.data""")
    ...
    def get_cn(self, idx_atom, tolerance):
        mic_coords = mic(self.xyz_coords[idx_atom], self.xyz_coords, nb.typed.List(self.cl), len(self.xyz_coords))
        dists = np.linalg.norm(mic_coords, axis=1)
        return np.sum(((dists != 0) & (dists <= tolerance)))
    ...
    def remove_over_coord(self, wanted_CN, tolerance, max_remove_over):
        ...
        #     num_removed, idx_current_atom = 0, len(self.atoms)
        #     while num_removed <= len(to_delete)-max_remove_over:
        #         if CN[idx_current_atom] > wanted_CN[self.atoms[idx_current_atom]]:
        #             self.atoms = np.delete(self.atoms, idx_current_atom)
        #             self.xyz_coords = np.delete(self.xyz_coords, idx_current_atom, axis=0)
        #             num_removed += 1
