...
from LAMMPS_interface_legacy import Lammps_interface
...
def set_i(current_atoms, current_xyz, cl, weight_z=False) -> int:
    possible_choices = {}
    # i = []
    if len(current_atoms) != 0:
        ...
        if len(keys) == 0:
            return np.random.choice(np.arrange(len(current_atoms)))
...
def slice(current_atoms, current_xyz_coords, limits):
    ...
    for idx, xyz in enumerate(current_xyz_coords):
        ...
        z_bound = limits[3][idx_x, idx_y] + np.min(current_xyz_coords[:, 2]) + 1
        if z > z_bound:
            to_delete.append(idx)
    
    current_atoms = np.delete(current_atoms, to_delete)
    current_xyz_coords = np.delete(current_xyz_coords, to_delete, axis=0)
    write_xyz("after_slice", current_atoms, current_xyz_coords)
    return current_atoms, current_xyz_coords

def generate_crystal(total_num_atoms, alpha, n_m):
    atoms = np.empty(0)
    ...
    upper_limit = make_Fourier_function(cl[0], cl[1], 500, alpha, n_m, n_m)
    ...
    number_write = 0
    TOTAL_DESIRED_ATOM, new_total_atoms = total_num_atoms, 0
    hit_final = False
    while new_total_atoms < TOTAL_DESIRED_ATOM:
        ...
        if made_placement:
            ...
            atoms, xyz_coords, dims = opt_tool.opt_struc("anneal", steps=250, start_T=298, final_T=298, FF="BKS")
            atoms, xyz_coords, dims = opt_tool.opt_struc("anneal", steps=max_steps, start_T=start_T, final_T=final_T, FF=FF,
                                                         removal="everything", max_num_rings=0, max_remove_over=0)
        ...
        if new_total_atoms == TOTAL_DESIRED_ATOM:
            ...
            if hit_final:
                # atoms, xyz_coords, dims = opt_tool.opt_struc(FF="ReaxFF")
                atoms, xyz_coords, dims = opt_tool.opt_struc("anneal", steps=750, start_T=500, final_T=4000, FF="BKS")
                atoms, xyz_coords, dims = opt_tool.opt_struc("final", steps=1000, start_T=298, final_T=298, 
                                                             removal="over-coord", max_num_rings=0, max_remove_over=0,
                                                             FF="ReaxFF")
            else:
                atoms, xyz_coords, dims = opt_tool.opt_struc("anneal", steps=250, start_T=298, final_T=298, FF="BKS")
                atoms, xyz_coords, dims = opt_tool.opt_struc("anneal", steps=750, start_T=298, final_T=1000, 
                                                             removal="over-coord", max_num_rings=0, max_remove_over=0,
                                                             FF="BKS")
            hit_final = True
            new_total_atoms = len(atoms)
